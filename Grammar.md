...
> 鏈串指定式最常犯的錯誤是 var a = b = 42。如果在執行時沒有 var b 來正式宣告 b ，那邊 var a = b = 42 不會直接宣告 b 。若有 strict 模式可能會擲出一個錯誤或是直接宣告一個全域變數。
...
JS 有一個鮮為人知也不鼓勵使用的方法，「帶了標籤的述句」foo 是述句 bar()的一個標籤，而 JS 支援了標籤跳躍。continue 與 break 述句都能選擇性地接受一個特定標籤而那會使程式流程以跳躍方式進行。
...
跳過了奇數乘積的 3 1 迭代，帶有標籤的迴圈跳躍也跳過了迭代的 1 1 及 2 2 。標籤迴圈的跳躍比較有用的地方是與 break\_搭配，從內層迴圈你想要中斷的地方跳離外層迴圈會比較有用一些。如果沒有帶標籤的 break，同樣邏輯有時寫出來可能會有點怪。
...
指定式周圍的()是必要的，因為 && 優先序比 = 還要高，若沒有 () 來迫使該繫結優先發生，則運算式就會被視為 (str && matches) = str.match..。 但(str && matches)結果不會是個變數，會是一個值(undefined) 無法在一個 = 指定式的左邊。
